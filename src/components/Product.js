...
const Product = ({ post }) => {
    // FIXED: Select the cart state directly (the array)
    const posts = useSelector((state) => state.cart);  // Because initialState = []
    ...
    return (
        ...
        hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl border hover:shadow-[0_35px_60px_-15px_rgba(0,0,0,0.3)]">
            <div><p className="text-gray-900 font-semibold text-lg text-left truncate w-40 mt-1">{post.title}</p></div>
